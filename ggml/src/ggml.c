...
static const struct ggml_type_traits type_traits[GGML_TYPE_COUNT] = {
    ...
    [GGML_TYPE_Q1A] = {
        ...
        .type_size                = sizeof(int8_t),
        .is_quantized             = true,
        .vec_dot                  = (ggml_vec_dot_t) ggml_vec_dot_q1a_i8_s,
        .vec_dot_type             = GGML_TYPE_I8_S,
        .nrows                    = 1,
    },
    [GGML_TYPE_I8_S] = {
...
#if defined(GGML_NAN_DEBUG)
bool save_tensor_to_file(const char* tensor_name, 
                        enum ggml_type type, 
    ...
    for (size_t i = 0; i < num_dims; i++) {
        fprintf(file, "%zu%s", dimensions[i], i < num_dims - 1 ? ", " : "");
    }
    fprintf(file, "]\n");
    
    const char* type_str = "unknown";
    size_t element_size = 0;
    const size_t VALUES_PER_LINE = 1024; // 每行输出的数值数量
    ...
    switch (type) {
        case GGML_TYPE_F32: {
            ...
            for (size_t i = 0; i < element_count; i++) {
                fprintf(file, "%.6f ", float_data[i]);
                // 每输出VALUES_PER_LINE个数值就换行
                if ((i + 1) % VALUES_PER_LINE == 0) {
                    fprintf(file, "\n");
                }
            }
            // 如果最后一行没有满，也换行
            if (element_count % VALUES_PER_LINE != 0) {
                fprintf(file, "\n");
            }
            break;
        }
...
static void ggml_compute_forward_mul_mat_one_chunk(
    ...
    const float * scale_q1a  = (float * )((uint8_t*) (src0->data) + (ne00 * ne01 / 8));
    ...
    const int32_t * act_sums   = (const int32_t*) ((const char *) act_scales + (ne11) * sizeof(float));
        ...
        printf("ir0_start: %ld ir0_end:%ld\n", ir0_start, ir0_end);
        printf("ir1_start: %ld ir1_end:%ld\n", ir1_start, ir1_end);
        printf("ne00: %ld, ne01: %ld\n", ne00, ne01);
        printf("ne12: %ld, ne02: %ld\n", ne12, ne02);
        printf("ne13: %ld, ne03: %ld\n", ne13, ne03);
        printf("num_rows_per_vec_dot: %ld, row_size: %ld\n", num_rows_per_vec_dot, row_size);
    #endif
    ...
    for (int64_t iir1 = ir1_start; iir1 < ir1_end; iir1 += blck_1) {
        for (int64_t iir0 = ir0_start; iir0 < ir0_end; iir0 += blck_0) {
            for (int64_t ir1 = iir1; ir1 < iir1 + blck_1 && ir1 < ir1_end; ir1 += num_rows_per_vec_dot) {
                ...
                // hack
                int src1_col_de_bias = 0;
                if (src0->type == GGML_TYPE_I2_S) {
                    src1_col_de_bias = (i11 * nb11 / 4);
                } else if (src0->type == GGML_TYPE_Q1A) {
                    src1_col_de_bias = (i11 * nb11 / 4);  // 表示激活，除以4是32bit量化到了8bit
                }
                const char * src1_col_de = (const char*)wdata + src1_col_de_bias;
                ...
                // #ifdef GGML_NAN_DEBUG
                //     printf("nb1: %ld ,nb0: %ld, num_rows_per_vec_dot: %ld\n", nb1, nb0, num_rows_per_vec_dot);
                //     printf("iir1: %ld ,iir0: %ld ,ir1: %ld, memcpy_size: %ld \n", iir1, iir0, ir1, (MIN(iir0 + blck_0, ir0_end) - iir0) * sizeof(float));
                //     for (int i = 0; i < 32; i++) {
                //         printf("tmp[%d] = %.6f\t", i, tmp[i]);
                //         if ((i+1) % 6 == 0) printf("\n");
                //     }
                //     printf("\n");
                //     // for (int i = 0; i < 256; i++) {
                //     //     printf("src1_col_de[%d] = %d\t", i, src1_col_de[i]);
                //     //     if ((i+1) % 6 == 0) printf("\n");
                //     // }
                //     // printf("\n");
...
static void check_float16_nan(const float *data, size_t len, const char *name) {
    for (size_t i = 0; i < len; i++) {
        float val = GGML_FP16_TO_FP32(((const ggml_fp16_t*)data)[i]);
        if (isnan(val)) {
            fprintf(stderr, "NaN detected in %s at index %zu \n", name, i);
        }
    }
}

static void check_float_nan(const float *data, size_t len, const char *name) {
    for (size_t i = 0; i < len; i++) {
        if (isnan(data[i])) {
            fprintf(stderr, "NaN detected in %s at index %zu \n", name, i);
        }
...
static void check_float16_large(const float *data, size_t len, const char *name) {
    int count = 0;
    for (size_t i = 0; i < len; i++) {
        float val = GGML_FP16_TO_FP32(((const ggml_fp16_t*)data)[i]);
        if (val > 500.0f && count < 10) {
            ++count;
            fprintf(stderr, ">500 detected in %s at index %zu \n", name, i);
        }
    }
}

static void check_float_large(const float *data, size_t len, const char *name) {
    int count = 0;
    for (size_t i = 0; i < len; i++) {
        if (data[i] > 500.0f && count < 10) {
            ++count;
            fprintf(stderr, ">500 detected in %s at index %zu \n", name, i);
        }
    }
...
UseGgmlGemm2:;
    ...
    #ifdef GGML_NAN_DEBUG
        if (src0->data != NULL) {
            size_t src0_len = src0->ne[0] * src0->ne[1] * src0->ne[2] * src0->ne[3];
            printf("src0->name: %s \n", src0->name);
            if (src0->type == GGML_TYPE_F16) {
                check_float16_nan((const float*)src0->data, src0_len, "src0->data (F16)");
                check_float16_large((const float*)src0->data, src0_len, "src0->data (F16)");
            }
        }
        if (src1->data != NULL) {
            size_t src1_len = src1->ne[0] * src1->ne[1] * src1->ne[2] * src1->ne[3];
            printf("src1->name: %s \n", src1->name);
            if (src1->type == GGML_TYPE_F32) {
                check_float_nan((const float*)src1->data, src1_len, "src1->data");
                check_float_large((const float*)src1->data, src1_len, "src1->data");
            }
        }
    #endif
    ...
    while (current_chunk < nchunk0 * nchunk1) {
        ...
        if (dst->data != NULL) {
            ...
            if (dst->type == GGML_TYPE_F32) {
                printf("dst->name: %s \n", dst->name);
                check_float_nan((const float*)dst->data, dst_len, "dst->data");
                check_float_large((const float*)dst->data, dst_len, "dst->data");
            }
            if (strcmp(dst->name, "Qcur-0") == 0) {
                printf("dst->data (前10个值): ");
                switch (dst->type) {
                    case GGML_TYPE_F32: {
                        const float* data = (const float*)dst->data;
                        for (int i = 0; i < 10 && i < dst_len; i++) {
                            printf("%.6f ", data[i]);
                        }
                        break;
                    }
                    case GGML_TYPE_F16: {
                        const uint16_t* data = (const uint16_t*)dst->data;
                        for (int i = 0; i < 10 && i < dst_len; i++) {
                            // 将FP16转换为FP32再打印
                            uint32_t f32_bits = ((data[i] & 0x7FFF) << 13) | ((data[i] & 0x8000) << 16);
        ...
        if (strcmp(dst->name, "kq-0") == 0) {
            ...
            save_tensor_to_file(dst->name, dst->type, 
                                dst->ne, ggml_n_dims(dst), 
                                dst->data, dst_len, 
                                file_name);
            exit(EXIT_FAILURE);
        }
...
static void ggml_compute_forward_soft_max_f32(
    ...
    for (int i1 = ir0; i1 < ir1; i1++) {
        ...
        for (int i = 0; i < nc; ++i) {
            #ifdef GGML_NAN_DEBUG
                printf("wp[%d] = %f\n", i, wp[i]);
                printf("wp address: %d\n", (nc + CACHE_LINE_SIZE_F32) * ith);
                printf("sp[%d] = %f\n", i, sp[i]);
                printf("sp address: %d\n", i1*src0->nb[1]);
                printf("dp[%d] = %f\n", i, dp[i]);
                printf("dp address: %d\n", i1*dst->nb[1]);
                printf("slope = %f\n", slope);
                printf("GGML_FP16_TO_FP32(mp_f16[%d]) = %f\n", i, GGML_FP16_TO_FP32(mp_f16[i]));
                printf("mp_f16[i] = %f\n", i, mp_f16[i]);
                printf("mp_f32[i] = %f\n", i, mp_f32[i]);
                printf("params->wdata[%d] = %f\n", i, ((float*)params->wdata)[i]);
                exit(0);
            #endif
            assert(!isnan(wp[i]));
        }
...
struct ggml_cgraph * ggml_new_graph(struct ggml_context * ctx) {
    // printf("ggml_new_graph_custom 1\n");
...
struct ggml_cgraph * ggml_graph_dup(struct ggml_context * ctx, struct ggml_cgraph * cgraph) {
    // printf("ggml_new_graph_custom 2\n");
...
static thread_ret_t ggml_graph_compute_secondary_thread(void* data) {
    ...
    while (true) {
        ...
        if (state->pending) {
            state->pending = false;
            // printf("ggml_graph_compute_thread 1 \n");
...
enum ggml_status ggml_graph_compute(struct ggml_cgraph * cgraph, struct ggml_cplan * cplan) {
    ...
    if (threadpool == NULL) {
        ...
        // printf("threadpool 1 \n");
        // printf("cplan->work_data[%d] = %f\n", 0, ((float*)cplan->work_data)[0]);
    } else {
    ...
    if (n_threads > 1) {
        #pragma omp parallel num_threads(n_threads)
        {
            ...
            // printf("ggml_graph_compute_thread 2 \n");
            ggml_graph_compute_thread(&threadpool->workers[omp_get_thread_num()]);
        }
    } else {
        // printf("ggml_graph_compute_thread 3 \n");
        // struct ggml_compute_state * state_print = (struct ggml_compute_state *) (&threadpool->workers[0]);
        // struct ggml_threadpool    * tp_print    = state_print->threadpool;
        // const struct ggml_cgraph * cgraph_print = tp_print->cgraph;
        // const struct ggml_cplan  * cplan_print  = tp_print->cplan;
        // struct ggml_compute_params params_print = {
        //     /*.ith       =*/ state_print->ith,
        //     /*.nth       =*/ atomic_load_explicit(&tp_print->n_threads_cur, memory_order_relaxed),
        //     /*.wsize     =*/ cplan_print->work_size,
        //     /*.wdata     =*/ cplan_print->work_data,
        //     /*.threadpool=*/ tp_print,
        // };
        // printf("threadpool->workers[0]->threadpool->cplan:%f \n", 
        //             (float *)((&params_print)->wdata));
    ...
    // printf("ggml_graph_compute_thread 4 \n");
    ggml_graph_compute_thread(&threadpool->workers[0]);
#endif
...
struct ggml_cgraph * ggml_graph_import(const char * fname, struct ggml_context ** ctx_data, struct ggml_context ** ctx_eval) {
    ...
    // populate result
    {
        ...
        // printf("ggml_new_graph_custom 3\n");
...
enum ggml_opt_result ggml_opt_resume(
        ...
        struct ggml_opt_context * opt,
        struct ggml_tensor * f) {
    // printf("ggml_new_graph_custom 4\n");
...
size_t ggml_quantize_chunk(
    ...
    switch (type) {
        ...
        case GGML_TYPE_Q1A:     result = quantize_q1a(src + start, (char *) dst + start_row * row_size, nrows, n_per_row, imatrix); break;
    ...
    } else if (type == GGML_TYPE_Q1A) {
        result = nrows * row_size / 8 + 32;
    } else {
